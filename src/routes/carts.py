...
def add_movie_to_cart(
    ...
    existing_item = db.query(CartItemModel).filter(CartItemModel.cart_id == cart.id,
                                                   CartItemModel.movie_id == cart_item.movie_id).first()
    if existing_item:
        raise HTTPException(
    ...
    return CartItemResponseSchema(
        ...
        movie_id=new_cart_item.movie_id,
        name=movie.name,
...
def remove_movie_from_cart(
    ...
    existing_item = db.query(CartItemModel).filter(CartItemModel.cart_id == cart.id,
                                                   CartItemModel.id == cart_item_id).first()
...
def get_cart(
    ...
    for item in cart_items:
        ...
        if movie:
            cart_item_responses.append(CartItemResponseSchema(
                cart_id=cart.id,
                name=movie.name,
